...
const videoTabs = [
  { title: "HRS Package", url: "/Zone-03/Zone03- HRS.webm" },
  { title: "HP Storage", url: "/Zone-03/Zone03- Storage.webm" },
  { title: "Pilot Area", url: "/Zone-03/zone03- Open pilots.webm" },
  { title: "Chiller Area", url: "/Zone-03/zone03- chiller.webm" },
  { title: "Overall Plan", url: "/Zone-03/zone03- fire tank.webm" },
];
      ...
      <ErrorCatcher onError={err => { setHasError(true); setErrorMsg(err?.toString() || "Unknown error"); }}>
        {hasError ? (
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <div className="text-gray-500 text-center text-sm mt-2">{errorMsg || "Please check your Zone03.glb file or browser console for details."}</div>
          </div>
        ) : (
          children
        )}
...
class ErrorCatcher extends React.Component {
  ...
  componentDidCatch(error, info) {
    ...
    console.error('Zone 3 Model Error:', error, info);
  }
  render() {
...
function ZoneModel({ url }) {
  ...
  return <Center position={[0,0,0]}><primitive object={scene} scale={4} /></Center>;
}
...
export default function Zone3Tabs() {
  const leftTabs = videoTabs.slice(0, 3);
  const rightTabs = videoTabs.slice(3, 6);
  const [videoIndex, setVideoIndex] = useState(null);
  const zoneTitle = "Zone 3";
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-100 via-white to-blue-200 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full flex flex-row justify-between items-center px-4 mb-4 py-4 bg-white/80 rounded-2xl shadow-lg border border-blue-100 animate-fadein">
        ...
        <span className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight drop-shadow-lg">Zone 3</span>
        <button
          className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-blue-100 to-blue-300 text-blue-700 rounded-xl shadow hover:from-blue-200 hover:to-blue-400 hover:scale-105 border-2 border-blue-200 font-bold text-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400"
          style={{ boxShadow: '0 4px 24px 0 rgba(59, 130, 246, 0.10)' }}
      ...
      <div className="relative z-10 mb-10 w-full max-w-7xl h-[600px] bg-white/90 rounded-3xl shadow-2xl border-2 border-blue-100 flex flex-row items-center justify-center animate-fadein" style={{ boxShadow: '0 8px 32px 0 rgba(31, 38, 135, 0.18)' }}>
        ...
        <div className="flex-1 h-full flex items-center justify-center min-w-[700px] relative">
          {videoIndex === null && (
            <ModelErrorBoundary>
              <Canvas camera={{ position: [-250, 20, 80], fov: 50, far: 10000, near: 0.1 }} style={{ width: '100%', height: '100%' }}>
                <ambientLight intensity={0.8} />
                <directionalLight position={[5, 5, 5]} intensity={0.5} />
                <ZoneModel url="/Zone03.glb" />
                <OrbitControls 
                  enablePan={true} 
                  enableZoom={true} 
                  enableRotate={true} 
          ...
          <button className="absolute right-4 bottom-4 px-6 py-3 text-lg font-semibold bg-gradient-to-r from-blue-400 to-blue-600 text-white rounded-xl shadow-lg hover:from-blue-500 hover:to-blue-700 hover:scale-105 active:scale-95 border-2 border-blue-200 transition-all duration-200" onClick={() => navigate('/zones/2')}>
            Go to Zone 2
          </button>
        </div>
